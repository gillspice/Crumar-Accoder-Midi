...
uint8 MIDI2_UART_initVar = 0u;
    ...
    volatile uint8 MIDI2_UART_txBuffer[MIDI2_UART_TX_BUFFER_SIZE];
    ...
    uint8 MIDI2_UART_errorStatus = 0u;
    volatile uint8 MIDI2_UART_rxBuffer[MIDI2_UART_RX_BUFFER_SIZE];
...
*  MIDI2_UART_Start() sets the initVar variable, calls the
*  MIDI2_UART_Init() function, and then calls the
*  MIDI2_UART_Enable() function.
*
* Parameters:
*  None.
*
...
*  and set to one (1u) the first time MIDI2_UART_Start() is called. This
*  allows for component initialization without re-initialization in all
*  subsequent calls to the MIDI2_UART_Start() routine.
*
* Reentrant:
*  No.
*
...
*  dialog settings. It is not necessary to call MIDI2_UART_Init() because
*  the MIDI2_UART_Start() API calls this function and is the preferred
...
*  to call MIDI2_UART_Enable() because the MIDI2_UART_Start() API
...
void MIDI2_UART_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if (MIDI2_UART_TX_INTERRUPT_ENABLED)
            MIDI2_UART_ClearPendingTxInt(); /* Clear history of TX_NOT_EMPTY */
...
void  MIDI2_UART_WriteControlRegister(uint8 control) 
{
    ...
    *      MIDI2_UART_RX_STS_FIFO_NOTEMPTY    Interrupt on byte received.
    *      MIDI2_UART_RX_STS_PAR_ERROR        Interrupt on parity error.
    *      MIDI2_UART_RX_STS_STOP_ERROR       Interrupt on stop error.
    *      MIDI2_UART_RX_STS_BREAK            Interrupt on break.
    *      MIDI2_UART_RX_STS_OVERRUN          Interrupt on overrun error.
    *      MIDI2_UART_RX_STS_ADDR_MATCH       Interrupt on address match.
    *      MIDI2_UART_RX_STS_MRKSPC           Interrupt on address detect.
    *
    * Return:
    *  None.
    *
    ...
    *  MIDI2_UART_rxBufferLoopDetect - cleared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 MIDI2_UART_ReadRxData(void) CYREENTRANT
    {
        ...
        locRxBufferRead  = MIDI2_UART_rxBufferRead;
        locRxBufferWrite = MIDI2_UART_rxBufferWrite;

        if( (MIDI2_UART_rxBufferLoopDetect != 0u) || (locRxBufferRead != locRxBufferWrite) )
        {
            rxData = MIDI2_UART_rxBuffer[locRxBufferRead];
            locRxBufferRead++;

            if(locRxBufferRead >= MIDI2_UART_RX_BUFFER_SIZE)
            {
                locRxBufferRead = 0u;
            }
            /* Update the real pointer */
            MIDI2_UART_rxBufferRead = locRxBufferRead;
    ...
    *  MIDI2_UART_RX_STS_FIFO_NOTEMPTY.
    *  MIDI2_UART_RX_STS_FIFO_NOTEMPTY clears immediately after RX data
    *  register read.
    *
    * Global Variables:
    ...
    *
    * Summary:
    *  Returns the last received byte of data. MIDI2_UART_GetChar() is
    ...
    *******************************************************************************/
    uint16 MIDI2_UART_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        MIDI2_UART_DisableRxInt();
        locErrorStatus = (uint16)MIDI2_UART_errorStatus;
        MIDI2_UART_errorStatus = 0u;
        MIDI2_UART_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | MIDI2_UART_ReadRxData() );
    #else
    ...
    *******************************************************************************/
                                                            ...
                                                            CYREENTRANT
    {
        ...
        if(MIDI2_UART_rxBufferRead == MIDI2_UART_rxBufferWrite)
        {
            if(MIDI2_UART_rxBufferLoopDetect != 0u)
            {
                size = MIDI2_UART_RX_BUFFER_SIZE;
            }
            else
            {
                size = 0u;
        ...
        {
            size = (MIDI2_UART_RX_BUFFER_SIZE - MIDI2_UART_rxBufferRead) + MIDI2_UART_rxBufferWrite;
        }
    ...
    *   MIDI2_UART_TX_STS_COMPLETE        Interrupt on TX byte complete
    *   MIDI2_UART_TX_STS_FIFO_EMPTY      Interrupt when TX FIFO is empty
    *   MIDI2_UART_TX_STS_FIFO_FULL       Interrupt when TX FIFO is full
    *   MIDI2_UART_TX_STS_FIFO_NOT_FULL   Interrupt when TX FIFO is not full
    *
    * Return:
    *  None.
    *
    ...
    *******************************************************************************/
    void MIDI2_UART_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(MIDI2_UART_initVar != 0u)
        {
            ...
            {
                if(MIDI2_UART_txBufferWrite >= MIDI2_UART_TX_BUFFER_SIZE)
                {
    ...
    *******************************************************************************/
    void MIDI2_UART_PutChar(uint8 txDataByte) CYREENTRANT
    {
        ...
        */
        ...
        #if ((MIDI2_UART_TX_BUFFER_SIZE > MIDI2_UART_MAX_BYTE_VALUE) && (CY_PSOC3))
            /* Disable TX interrupt to protect variables from modification */
            MIDI2_UART_DisableTxInt();
        #endif /* (MIDI2_UART_TX_BUFFER_SIZE > MIDI2_UART_MAX_BYTE_VALUE) && (CY_PSOC3) */

            locTxBufferWrite = MIDI2_UART_txBufferWrite;
            locTxBufferRead  = MIDI2_UART_txBufferRead;
        ...
        while( (locTxBufferWrite < locTxBufferRead) ? (locTxBufferWrite == (locTxBufferRead - 1u)) :
                                ...
                                (uint8)(MIDI2_UART_TX_BUFFER_SIZE - 1u)) );
        ...
        {
            if(locTxBufferWrite >= MIDI2_UART_TX_BUFFER_SIZE)
            {
                locTxBufferWrite = 0u;
            }
            /* Add to the software buffer. */
            MIDI2_UART_txBuffer[locTxBufferWrite] = txDataByte;
            locTxBufferWrite++;
            ...
            MIDI2_UART_txBufferWrite = locTxBufferWrite;

        #if ((MIDI2_UART_TX_BUFFER_SIZE > MIDI2_UART_MAX_BYTE_VALUE) && (CY_PSOC3))
            MIDI2_UART_EnableTxInt();
        #endif /* (MIDI2_UART_TX_BUFFER_SIZE > MIDI2_UART_MAX_BYTE_VALUE) && (CY_PSOC3) */

            if(0u != (MIDI2_UART_TXSTATUS_REG & MIDI2_UART_TX_STS_FIFO_EMPTY))
            {
                /* Trigger TX interrupt to send software buffer */
                MIDI2_UART_SetPendingTxInt();
            }
        }
    ...
    *******************************************************************************/
    void MIDI2_UART_PutString(const char8 string[]) 
    {
        ...
        if(MIDI2_UART_initVar != 0u)
        {
            ...
            {
                MIDI2_UART_PutChar((uint8)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(MIDI2_UART_initVar != 0u)
        {
            ...
            {
                MIDI2_UART_PutChar(string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (MIDI2_UART_TX_BUFFER_SIZE - MIDI2_UART_txBufferRead) +
                    MIDI2_UART_txBufferWrite;
        }
    ...
    *   MIDI2_UART_SEND_BREAK - Initialize registers for break, send the Break
    *       signal and return immediately.
    *   MIDI2_UART_WAIT_FOR_COMPLETE_REINIT - Wait until break transmission is
    *       complete, reinitialize registers to normal transmission mode then return
    *   MIDI2_UART_REINIT - Reinitialize registers to normal transmission mode
    *       then return.
    *   MIDI2_UART_SEND_WAIT_REINIT - Performs both options: 
    *      MIDI2_UART_SEND_BREAK and MIDI2_UART_WAIT_FOR_COMPLETE_REINIT.
    ...
    *     SendBreak(0);     - initialize Break signal transmission
    *         Add your code here to use CPU time
    *     When interrupt appear with MIDI2_UART_TX_STS_COMPLETE status:
    *     SendBreak(2);     - complete Break operation
    *
    * Side Effects:
    *  The MIDI2_UART_SendBreak() function initializes registers to send a
    *  break signal.
    ...
    void MIDI2_UART_SendBreak(uint8 retMode) 
    {
        ...
        if(MIDI2_UART_initVar != 0u)
        {
                ...
                (retMode == MIDI2_UART_SEND_WAIT_REINIT) )
            {
                ...
                #if(MIDI2_UART_TXCLKGEN_DP)
                    txPeriod = MIDI2_UART_TXBITCLKTX_COMPLETE_REG;
            ...
            #if(MIDI2_UART_TXCLKGEN_DP)
                MIDI2_UART_TXBITCLKTX_COMPLETE_REG = txPeriod;
            #else
    ...
    * Parameters:
    *  addressMode: 
    *       MIDI2_UART_SET_SPACE - Configure the transmitter to send the next
    *                                    byte as a data.
    *       MIDI2_UART_SET_MARK  - Configure the transmitter to send the next
    *                                    byte as an address.
    *
    * Return:
    *  None.
    *
    * Side Effects:
    *  This function sets and clears MIDI2_UART_CTRL_MARK bit in the Control
    *  register.
    *
    *******************************************************************************/

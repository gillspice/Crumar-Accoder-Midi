...
extern const uint8 CYCODE USB_DEVICE0_CONFIGURATION0_DESCR[133u];
...
extern const uint8 CYCODE USB_STRING_DESCRIPTORS[185u];
...
void USB_Config(uint8 clearAltSetting) CYREENTRANT;
void USB_ConfigAltChanged(void)        ;
void USB_ConfigReg(void)               CYREENTRANT;
void USB_EpStateInit(void)             ;


const T_USB_LUT CYCODE *USB_GetConfigTablePtr(uint8 confIndex)CYREENTRANT;
const T_USB_LUT CYCODE *USB_GetDeviceTablePtr(void)           ;
#if (USB_BOS_ENABLE)
    const T_USB_LUT CYCODE *USB_GetBOSPtr(void)               ;
#endif /* (USB_BOS_ENABLE) */
const uint8 CYCODE *USB_GetInterfaceClassTablePtr(void)                    CYREENTRANT;uint8 USB_ClearEndpointHalt(void)                                          ;
...
***************************************/
    ...
    void USB_MIDI_IN_EP_Service(void)  CYREENTRANT;
